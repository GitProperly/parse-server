...
var ParseLiveQueryServer = function () {
  ...
  function ParseLiveQueryServer(server, config) {
    ...
    // Initialize cache
    if (config.cacheAdapter instanceof _RedisCacheAdapter2.default) {
      var cacheControllerAdapter = (0, _AdapterLoader.loadAdapter)(config.cacheAdapter, _InMemoryCacheAdapter.InMemoryCacheAdapter, { appId: appId });
      this.cacheController = new _CacheController.CacheController(cacheControllerAdapter, appId);
    }
  ...
  _createClass(ParseLiveQueryServer, [{
              ...
              try {
                var _loop5 = function _loop5() {
                  ...
                  _node2.default.Promise.when(originalACLCheckingPromise, currentACLCheckingPromise).then(function (isOriginalMatched, isCurrentMatched) {
                    ...
                    var ssToken = client.getSubscriptionInfo(requestId).sessionToken;
                    _this3.sessionTokenCache.getUserId(ssToken).then(function (userId) {
                      return _this3.cacheController.role.get(ssToken).then(function (cUser) {
                        if (cUser) return cUser;
                        return new _node2.default.Query(_node2.default.User).equalTo("objectId", userId).limit(10000).find({ useMasterKey: true }).then(function (user) {
                          if (!user || !user.length) return undefined;
                          _this3.cacheController.role.put(ssToken, user[0]);
                          return JSON.parse(JSON.stringify(user[0]));
                        });
                      });
                    }).then(function (user) {
                      var result = currentParseObject;
                      (_this3.queryMiddleware || []).forEach(function (ware) {
                        return result = ware(result.className, [result], { user: { getTeams: function getTeams() {
                              return user.teams;
                            } } })[0];
                      });
      ...
      return this.sessionTokenCache.getUserId(subscriptionSessionToken).then(function (userId) {
        ...
        return new _node2.default.Promise(function (resolve, reject) {
          ...
          _this5.sessionTokenCache.getUserId(subscriptionSessionToken).then(function (userId) {
            ...
            if (_this5.cacheController && _this5.cacheController.adapter && _this5.cacheController.adapter instanceof _RedisCacheAdapter2.default) {
              return _this5.cacheController.role.get(user.id).then(function (roles) {
                if (roles != null) {
                ...
                return _this5._loadRoles(user).then(function (roles) {
                  _this5.cacheController.role.put(user.id, roles.map(function (role) {
                    return 'role:' + role;
                  }));
                  return roles;
                });
              ...
              return _this5._loadRoles(user);
            }
          }).then(function (roles) {
            // Finally, see if any of the user's roles allow them read access
            return resolve(!!~roles.findIndex(function (role) {
    ...
    key: '_loadRoles',
    value: function _loadRoles(user) {
      var _this6 = this;

      var query = new _node2.default.Query(_node2.default.Role);
      query.equalTo('users', user);
      query.limit(10000);
      return query.find({ useMasterKey: true }).then(function (roles) {
        ...
        var queriedRoles = {};
        return _this6._getAllRolesNamesForRoleIds(roleIDs, names, queriedRoles);
      });
    }
  }, {
    key: '_getAllRolesNamesForRoleIds',
    value: function _getAllRolesNamesForRoleIds(roleIDs, names, queriedRoles) {
      var _this7 = this;
      ...
      var ins = roleIDs.filter(function (roleId) {
        ...
        var role = new _node2.default.Role();
        role.id = roleId;
        return role;
      });
      if (ins.length === 0) {
        return Promise.resolve([].concat(_toConsumableArray(names)));
      }
      ...
      return query.find({ useMasterKey: true }).then(function (roles) {
        ...
        return _this7._getAllRolesNamesForRoleIds(ids, names, queriedRoles);
      }).then(function (names) {
        return Promise.resolve([].concat(_toConsumableArray(names)));
      });
    }
